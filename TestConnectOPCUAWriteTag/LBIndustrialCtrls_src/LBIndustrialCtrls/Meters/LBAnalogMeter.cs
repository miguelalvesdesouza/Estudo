...
namespace LBSoft.IndustrialCtrls.Meters
{
	...
	{		
        ...
        public double warningValue = 70;
        [
            Category("Analog Meter"),
            Description("Percent value for warning"),
            DefaultValue(70)
        ]
        ...
        public double criticalValue = 80;
        [
            Category("Analog Meter"),
            Description("percent value of the Critical"),
            DefaultValue(80)
        ]
        ...
        {
            if (this.listThreshold.Count==0)
            {
                LBMeterThreshold w = new LBMeterThreshold();
                w.Color = Color.Goldenrod;
                w.StartValue = 70;
                w.EndValue = 70;
                this.listThreshold.Add(w);
                LBMeterThreshold c = new LBMeterThreshold();
                c.Color = Color.Red;
                c.StartValue = 80;
                c.EndValue = 100;
                this.listThreshold.Add(c);
            }
            if ( this.listThreshold.Count == 2)
            {
                LBMeterThreshold ww = this.listThreshold[0];
                LBMeterThreshold cc = this.listThreshold[1];
                ww.StartValue = this.maxValue * this.warningValue / 100;
                ww.EndValue = this.maxValue * this.criticalValue / 100;
                cc.StartValue = this.maxValue * this.criticalValue / 100;
                cc.EndValue = this.maxValue;
            }
        }
		[Browsable(false)]
